#!/usr/bin/env tsx
/**
 * Mint tETH to any address on Sui testnet.
 *
 * Usage:
 *   pnpm tsx contracts/test_eth/mint.ts [amount_test_eth] [recipient]
 *
...
async function main() {
  const amountETH = parseFloat(process.argv[2] ?? "1");
  if (isNaN(amountETH) || amountETH <= 0 || amountETH > 10) {
    console.error("amount must be between 0 and 10 ETH");
    process.exit(1);
  }
  const amountRaw = BigInt(Math.round(amountETH * 10 ** DECIMALS));
  if (amountRaw > MAX_AMOUNT) {
    console.error(`max per call is 10 ETH (${MAX_AMOUNT} raw units)`);
    process.exit(1);
  }
  ...
  console.log(`Minting ${amountETH} tETH → ${recipient}`);
  ...
  if (privKey) {
    ...
    console.error(
      ...
      `  sui client call --package ${deployed.packageId} --module test_eth --function mint ` +
  ...
  tx.moveCall({
    target: `${deployed.packageId}::test_eth::mint`,
    arguments: [
  ...
  console.log(`✓ Minted ${amountETH} tETH`);
