#!/usr/bin/env tsx
/**
 * Mint tSUI to any address on Sui testnet.
 *
 * Usage:
 *   pnpm tsx contracts/test_sui/mint.ts [amount_test_sui] [recipient]
 *
...
async function main() {
  ...
  if (isNaN(amountSUI) || amountSUI <= 0 || amountSUI > 100) {
    console.error("amount must be between 0 and 100 SUI");
    process.exit(1);
  }
  const amountRaw = BigInt(Math.round(amountSUI * 10 ** DECIMALS));
  if (amountRaw > MAX_AMOUNT) {
    console.error(`max per call is 100 SUI (${MAX_AMOUNT} raw units)`);
    process.exit(1);
  }
  ...
  console.log(`Minting ${amountSUI} tSUI → ${recipient}`);
  ...
  if (privKey) {
    ...
    console.error(
      ...
      `  sui client call --package ${deployed.packageId} --module test_sui --function mint ` +
  ...
  tx.moveCall({
    target: `${deployed.packageId}::test_sui::mint`,
    arguments: [
  ...
  console.log(`✓ Minted ${amountSUI} tSUI`);
